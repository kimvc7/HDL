...
# the single most frankenstein line of code I've written in R so far
...
# lol nope we need something stronger to process the new dictionary of adversarial values.
# data_total$avg_adv_test_acc = sapply(strsplit(str_sub(data_total$adv_test_accs, start=2, end = -2),"\\s+|\n"),function(x) mean(as.numeric(x)) )



# dataset_name = "uci7"

# data_uci =  read_csv(paste0(paste0("results_ff",dataset_name),".csv"), col_names = c("stable","robust","robust_test","n_experiments",
#                                                           "train_size","batch_size","subset_ratio","avg_test_acc","dict_test_accs","adv_test_accs","std",
#                                                           "thetas","num_training_steps","iterations","w1_stability","w2_stability","w3_stability","logit_stability",
#                                                           "gini_stability","l2","l0","W1_non_zero","W2_non_zero","W3_non_zero","l1_size","l2_size","lr"))
# data_total = data_uci[complete.cases(data_uci),]

# calculate average adversarial statistics
...
#need compute from list
#avg_adv_test, logit_stability
...
train_y = data_total[, "gini_stability"]

#model10 = lm(gini_stability ~ (stable + robustness + l0_reg + sparsity)^3 +  batch_size + l2_reg, data = data_total)

model10 = lm(gini_stability ~ (stable + robustness + l0_reg)^3 + sparsity + batch_size + l2_reg, data = data_total)
summary(model10)
